/**
  ...
  *          Updated with Motor Safety Test & Error Control v2.0
  *
  *  Original bridge features maintained + new error management:
  *    1. CLR_ERR command clears latched Motor Safety errors
  *    2. ERROR_* messages from safety module reported to HMI
  *    3. Safety test status included in telemetry
  *    4. Full backward compatibility with original protocol
  ******************************************************************************
  */
#include "bt_motor.h"
#include <string.h>
...
static void cb_fault(Motor_Fault_t f)
{
    ...
    switch (f) {
        ...
        case MOTOR_FAULT_SAFETY_TEST:  s = "LOG:FAULT SAFETY_TEST_FAILED\r\n"; break;  /* NEW */
...
/* ------------------------------------------------------------------ */
/*  Telemetry frame (updated with safety test status)                 */
/* ------------------------------------------------------------------ */
static void bt_send_telemetry(BtMotor_t *bt)
{
    Motor_Handle_t *m = bt->motor;
    Motor_Safety_t *s = bt->safety;     /* NEW */
    ...
    int pwr = (st != MOTOR_STATE_IDLE && st != MOTOR_STATE_FAULT && st != MOTOR_STATE_SAFETY_TEST) ? 1 : 0;
    int flt = (st == MOTOR_STATE_FAULT || Motor_Safety_IsErrorLatched(s)) ? 1 : 0;  /* NEW: include safety */
    ...
    const char *sta = (st == MOTOR_STATE_FAULT) ? "FLT"
                    : (st == MOTOR_STATE_SAFETY_TEST) ? "TST"  /* NEW */
    ...
    /* Safety test status */
    int safety_state = (int)Motor_Safety_GetState(s);
    int safety_fault = (int)Motor_Safety_GetFault(s);

    char buf[350];  /* increased size for safety fields */
    snprintf(buf, sizeof(buf),
        ...
        "\"poles\":%d,\"mode\":\"%s\","
        "\"safe_st\":%d,\"safe_err\":%d}",  /* NEW: safety fields */
...
/* ------------------------------------------------------------------ */
/*  Command dispatch (updated with error clearing)                    */
/* ------------------------------------------------------------------ */
static void process_line(BtMotor_t *bt, char *l)
{
    ...
    else if (strcmp(l, "CLR_ERR") == 0)  /* NEW: Clear Safety Error */
    {
        Motor_Safety_ClearError(s);
        bt_send_ok(bt, "CLR_ERR");
        bt_send(bt, "LOG:SAFETY_ERROR_CLEARED=ON\r\n");
    }
...
/* ================================================================== */
/*  Public API (CORRECTED signature)                                  */
/* ================================================================== */
void BtMotor_Init(BtMotor_t *bt, Motor_Handle_t *motor, Motor_Safety_t *safety,
                  UART_HandleTypeDef *huart)
{
    memset(bt, 0, sizeof(*bt));
    bt->motor = motor;
    bt->safety = safety;  /* NEW */
    bt->huart = huart;
    ...
    bt_send(bt, "LOG:SAFETY_TEST_ENABLED=ON\r\n");  /* NEW */
}
